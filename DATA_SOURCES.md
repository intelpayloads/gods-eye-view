...
| **OpenSky Network** | Primary worldwide live-flight snapshot (through the provider server in the standalone app; read from the Dataforge world model's `world.aircraft` binding when a host sets the layer source to `world`) | Non-commercial research/education license | Schäfer et al., *"Bringing Up OpenSky"*, IPSN 2014 + opensky-network.org |
...
| **adsb.lol** | Military flights (through the provider server in the standalone app; read from the Dataforge world model's `world.military_aircraft` binding when a host sets the layer source to `world`) + aircraft traces | ODbL 1.0 | "adsb.lol" (ODbL) |
...
| **USGS** | Earthquakes (fetched directly in the standalone app; read from the Dataforge world model's `world.earthquakes` binding when a host sets the layer source to `world`) | U.S. public domain | "Data courtesy of the U.S. Geological Survey" |
...
- **USGS via the world model.** The earthquake layer has two sources and one attribution. Standalone, it fetches the USGS `all_day` GeoJSON feed from the browser. When a host embeds the application with `layerSources: { earthquakes: 'world' }` (the Dataforge client), the layer reads the same events from the Dataforge world model instead (`src/layers/earthquakes/worldSource.js`): the world model's `usgs` connector retains the feed and publishes it as `world.earthquakes`, and the adapter rebuilds the feed shape the layer already validates. It is still USGS data, so the USGS credit stays registered either way; the M2.5 cut and the depth colouring are unchanged.
...
- **OpenSky via the world model.** When a host embeds the application with `layerSources: { flights: 'world' }` (the Dataforge client), the flights layer reads its snapshot from the Dataforge world model instead of `/api/opensky` (`src/layers/flights/worldSource.js`): the world model's `opensky` connector retains the state vectors and publishes them as `world.aircraft`, and the adapter rebuilds the records the layer already admits. It is still OpenSky data under the same non-commercial terms, so the OpenSky credit stays registered either way. Selected-aircraft trails (`/api/opensky-track`) and adsbdb enrichment (`/api/adsbdb`) still go to the provider server on that path; the world model carries no last-contact time, origin country or vertical rate, and omits a state without a geometric height.
- **adsb.lol via the world model.** When a host embeds the application with `layerSources: { military: 'world' }` (the Dataforge client), the military layer reads its snapshot and its identity list from the Dataforge world model instead of `/api/adsblol/mil` (`src/layers/military/worldSource.js`): the world model's `adsblol` connector retains the `/v2/mil` snapshot and publishes it as `world.military_aircraft`, and the adapter rebuilds the records the layer already admits (type code and registration included). It is still adsb.lol data under ODbL, so the credit stays registered either way. Selected-aircraft traces (`/api/adsblol/trace`) still go to the provider server on that path; the projector omits a state without a geometric height (ground traffic without `alt_geom`) or with a MLAT / TIS-B height.
